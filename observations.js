...
function speciesCoordinates(data) {
  for (let i = 0; i < data.results.length; i++) {
    ...
    const formattedString = `"${observation.species_guess}" observed at coordinates (${observation.location})`;
...
function speciesCoordinates2(data) {
  data.results.forEach((observation) => {
    ...
    const observed_loc = observation.location;

    console.log(`"${species_guess}" observed at coordinates (${observed_loc})`);
  });
}
...
function speciesCoordinates3(data) {
  for (const observation of data.results) {
    ...
    const location = observation.location.replace(/,/, ', ');
...
function observationsByQualityGrade(data, qualityGrade) {
  const validQualityGrade = ['research', 'needs_id', 'casual', null, 'unspecified'];
  const lowercaseQualityGrade = qualityGrade ? qualityGrade.toLowerCase() : qualityGrade;
  if (!validQualityGrade.includes(lowercaseQualityGrade)) {
    throw new Error('Invalid Quality Grade value');
  }
  ...
  for (const observation of data.results) {
    ...
    if (observationQualityGrade === lowercaseQualityGrade) {
...
function observationsByQualityGrades(data, ...qualityGrades) {
  if (qualityGrades.length === 0) {
    throw new Error('At least one quality grade should be provided');
  }

  const validQualityGrades = ['research', 'needs_id', 'casual', null];

  for (const grade of qualityGrades) {
    if (grade !== null && !validQualityGrades.includes(grade.toLowerCase())) {
      throw new Error(`Invalid Quality Grade value: ${grade}`);
    }
  }

  const lowercaseQualityGrades = qualityGrades.map((grade) =>
    grade !== null ? grade.toLowerCase() : null
  );
  const matchingObservations = data.results.filter((observation) => {
    const observationQualityGrade = observation.quality_grade
      ? observation.quality_grade.toLowerCase()
      : observation.quality_grade;

    return lowercaseQualityGrades.includes(observationQualityGrade);
  });
...
function transformObservation(original) {
  const transformedObservation = {
    ...
    isExtinct: original.conservation_status?.status_name === 'extinct in the wild' || false,
    images:
...
function getObservationsByTaxa(data, ...taxaNames) {
  const validTaxa = new Set(['animalia', 'aves', 'insecta', 'plantae']);
  const validTaxonNames = taxaNames
    .filter((taxonName) => validTaxa.has(taxonName.toLowerCase()))
    .map((taxonName) => taxonName.toLowerCase());
  let filteredObservations = [];
  data.results.forEach((observation) => {
    const observationTaxon = observation.taxon.iconic_taxon_name.toLowerCase();
    if (validTaxonNames.includes(observationTaxon)) {
...
function getObservationsByLocation(data, options = {}) {
  ...
  if (
    ...
    dataArray = data.results.filter((element) => {
      if (
        (element.geojson.coordinates[1] >= options.lat.min &&
          element.geojson.coordinates[1] <= options.lat.max) ||
        (element.geojson.coordinates[0] >= options.lng.min &&
          element.geojson.coordinates[0] <= options.lng.min)
      ) {
        return element;
      }
    });
  } else if (options !== undefined && options.lat !== undefined && options.lng !== undefined) {
    dataArray = data.results.filter((element) => {
      if (
        element.geojson.coordinates[1] === options.lat &&
        element.geojson.coordinates[0] === options.lng
      ) {
        return element;
      }
    });
...
function getPlaceURLs(data) {
  const placeIds = data.results.flatMap((observation) => observation.place_ids);
  const uniquePlaceIds = [...new Set(placeIds)];
  const placeURLs = uniquePlaceIds.map(
    (placeId) => `https://www.inaturalist.org/observations?place_id=${placeId}`
  );
...
function extractSpeciesNames(data) {
  ...
  data.results.forEach((observation) => {
    if (observation.species_guess && !speciesNames.includes(observation.species_guess)) {
      speciesNames.push(observation.species_guess);
    }
  });
...
function extractSpeciesNames2(data) {
  ...
  data.results.forEach((observation) => {
    if (observation.species_guess) {
      speciesNamesSet.add(observation.species_guess);
    }
  });
