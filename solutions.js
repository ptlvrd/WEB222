...
function createLinkTag(rel, href) {
  ...
  href = href.trim();
  const specialChars = /[ \t\n\r]/;
  const relNeedsQuotes = specialChars.test(rel);
  const hrefNeedsQuotes = specialChars.test(href);
  let linkTag = '<link';
...
function parseDateString(value) {
  ...
  if (typeof value !== 'string') {
    throw new Error('Invalid format. Expected a string date in YYYY/MM/DD or DD/MM/YYYY format.');
  }

  const yyyyMmDdRegex = /^(\d{4})\/(\d{2})\/(\d{2})$/;
  const ddMmYyyyRegex = /^(\d{2})\/(\d{2})\/(\d{4})$/;

  const yyyyMmDdMatch = yyyyMmDdRegex.exec(value);
...
function formatTimes(...values) {
  // Replace this comment with your code...
  const validTimes = [];
  for (const timeString of values) {
...
function buildApiEndpoint(resourceType, resourceId, extraData) {
  ...
  if (!resourceType || !resourceId) {
    throw new Error('You need to provide both resourceType and resourceId.');
  }
